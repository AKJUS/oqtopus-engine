...
import numpy as np
import pytest
from estimator import Estimator, create_qiskit_operator
...
def init_estimator_instance():
    global est
    global logger
    logger = logging.getLogger(__name__)
    est = Estimator(logger)
    yield
    logger = None
    est = None
...
def test_preprocess():
    ...
    mapping_list = []
    qasms, op = est._preprocess(qasm_code, operators, basis_gates, mapping_list)
...
def test_postprocess():
    ...
    op = '[[["XX"], ["ZY"]], [[1.5], [1.2]]]'
    actual_expval, actual_stds = est._postprocess(counts_list, op)
...
def test_simple_circuits_with_random_op():
    ...
    # preprocess
    preprocessed_qasms, grouped_paulis = est._preprocess(
    ...
    # postprocess
    actual_expval, actual_stds = est._postprocess(l_counts, grouped_paulis)
...
def test_random_circuits_with_random_op():
    num_qubits = 4
    basis_gates = ["cx", "id", "rz", "sx", "x", "reset", "delay", "measure"]
    ...
    for i in range(1):
        ...
        # preprocess
        preprocessed_qasms, grouped_operators = est._preprocess(
            qasm_code, operators, basis_gates, mapping_list
        )
        ...
        # postprocess
        actual_expval, actual_stds = est._postprocess(l_counts, grouped_operators)

        # expectation
        pubs = [(circuit, observable)]
        aer_backend = AerSimulator()
        estimator_qiskit = BackendEstimatorV2(backend=aer_backend)
        job_qiskit = estimator_qiskit.run(pubs, precision=0.01)
        estimator_result = job_qiskit.result()
        expect_expval = estimator_result[0].data["evs"]
        expect_stds = estimator_result[0].data["stds"]
...
def test_random_circuits_with_transpiler():
    num_qubits = 4
    ...
    for i in range(1):
        ...
        # expect expval by qiskit AerSimulator based Estimator
        pubs = [(circuit.decompose(), observable)]
        aer_backend = AerSimulator()
        estimator_qiskit = BackendEstimatorV2(backend=aer_backend)
        job_qiskit = estimator_qiskit.run(pubs, precision=0.01)
        estimator_result = job_qiskit.result()
